...
from . import strategies_log as strategies
import logging
import time
...
class SpeculativeGenerator:
    ...
    def generate(
        ...
        while True:
            ...
            if log_show:
                logger.info(f"Draft output size: {draft_output.sequences.size()}, \n{draft_output.sequences}")
                logger.info(f"Draft cand probs: {draft_output.cand_probs}")
                logger.info(f"Draft past key values size: {len(draft_output.past_key_values)}")
            ...
            if log_show:
                logger.info(f"Verify output size: {verification_output.sequences.size()}, \n{verification_output.sequences}")
            log_show = False
        ...
        logger.info(f"Draft: {draft_time:.3f}, {invocation_count * self.strategy.max_draft_len / draft_time:.2f} | Verify: {verify_time:.3f}, speed: {invocation_count / verify_time:.2f}, {(input_ids.size(-1)-init_input_len)}/{invocation_count} = {(input_ids.size(-1)-init_input_len)/invocation_count:.2f}")
        # logger.info(f"")
